...
function App() {
  ...
  return <Box className="inventory-page">
    <Box className="page-heading"><Box><Typography className="eyebrow">ASSET INVENTORY</Typography><Typography variant="h1">Everything in its place.</Typography><Typography className="subtitle">Track, manage, and make the most of every asset your team owns.</Typography></Box><Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}><IconButton aria-label="Open account menu" disableRipple sx={{ '&:hover': { backgroundColor: 'transparent' } }} onClick={(event) => setProfileAnchor(event.currentTarget)}><Avatar sx={{ width: 40, height: 40 }}>{usernameInitial}</Avatar></IconButton>{isAdmin && <Button variant="contained" startIcon={<Add />} onClick={openCreate}>Add asset</Button>}<Menu anchorEl={profileAnchor} open={Boolean(profileAnchor)} onClose={closeProfile} anchorOrigin={{ vertical: 'top', horizontal: 'right' }} transformOrigin={{ vertical: 'bottom', horizontal: 'right' }}><MenuItem disabled>{username}</MenuItem><MenuItem onClick={() => { closeProfile(); void logout() }}>Sign out</MenuItem></Menu></Box></Box>
